...
\subsection*{Finding 7: PWM Integration---Cold CittaStore Recall is Integration-Ready, Equivalent to Analytic Baseline}
...
\item Seed 777: gap $= +0.1111$ (cold store favored; 1/9 quantization resolution, within noise)
\end{itemize}

The cold store's equivalence to the lens-transpose baseline confirms that the PWM write pathway is \emph{plumbing-correct} and dimensionally compatible.
...
Tier: confirm (functional, 3 seeds, integration verified); fail-on-margin on strict equivalence, but the margin caveat is that the cold store is untrained, so equivalence is consistent with vanilla initialization (gate L20, \cite{gate_L20_confirm}).

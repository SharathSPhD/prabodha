...
and transparency, not alignment gain.

\textbf{Recognition-gated hardening: the mechanism that resolves the negative (gemma-2-2b).}
The L21 negative above says that \emph{unconditional} activation steering does not help an already-aligned
model. But the doctrine's core claim is \emph{recognition-then-response}: recognize the input's harmful
signature first, and reinforce refusal only when a genuine attack is present. We test this directly on
gemma-2-2b against a battery of wrapped jailbreak prompts (DAN, roleplay, ignore-instructions), reading each
input's harmful projection onto the contrastive refusal direction at the band read-layer and gating the
defensive write on a threshold in the empirical benign/attack gap (gate L26-moat-proof). Three defenses:
...
\midrule
None & 0.50 & 0.00 \\
Unconditional hardening (brute force) & 0.25 & 1.00 \emph{(unusable)} \\
\textbf{Recognition-gated hardening} & \textbf{0.25} & \textbf{0.00} \\
\bottomrule
\end{tabular}
\end{center}

Recognition-gating cuts real jailbreaks in half---the \emph{same} attack reduction as brute-force
hardening---but at \emph{zero} benign collateral, whereas unconditional hardening refuses 100\% of benign
traffic and is therefore useless in deployment. Critically, jailbreak wrapping does not evade the gate: all
$12/12$ wrapped attacks were detected and $0/10$ benign prompts triggered a false refusal, because the
harmful signature lives at the \emph{activation} level, below what the surface prompt can disguise (benign
projections fall in $[-53,-18]$, attack projections in $[+4,+73]$---a clean, non-overlapping gap). A system
prompt cannot replicate this: it lives \emph{above} the input, so an attacker can see, override, or strip it,
whereas the recognition gate lives in the serving code's forward pass and is prompt-untouchable. This is the
positive counterpart to the L21 negative: activation defense works when it is \emph{recognition-gated}, not
when it is applied unconditionally. \emph{Caveats:} this is an exploratory result---$n=12$ attacks, $10$
benign, a single model and seed, and a refusal-phrase ASR metric; larger models, standardized probe suites
(e.g., garak), and multi-seed confirmation are the natural next steps. The primitives
(\texttt{harmful\_projection}, \texttt{recognition\_gate}, \texttt{act\_recognition\_gated}) ship in
\texttt{prabodha.steering.mechanisms}.

Future work combining band-steering with other methods (training, filtering, deeper recognition signals)
remains open.
...
\begin{itemize}
  \item At $\alpha=0.1$ (subtle dose): Prabodha detects $0.475$; jSpace final detects $0.2375$ (gap $+0.2375$, McNemar $p=2.1 \times 10^{-5}$, direction decisive). The gap misses the registered criterion (gap $\ge 0.3$) by $0.0625$. Verdict: \textbf{FAIL-ON-MARGIN}---the lens-site reading is twice as sensitive, but the margin is not recovered.
  \item At $\alpha=0.3$ (saturating dose): Both saturate (Prabodha band $1.0$, jSpace final $0.95$, gap $+0.05$, McNemar $p=0.125$). The necessity claim---that the lens is required at high doses---is \textbf{WITHDRAWN}: the registered falsifier fired (final $\ge$ band $- 0.1$). With both rates at ceiling, the comparison is uninformative at this dose---saturation, not evidence of equivalence (a null $p=0.125$ establishes neither).
  \item At $\alpha \in \{0.02, 0.05\}$: Both detectors fall to floor or equivocal levels; neither has signal.
\end{itemize}

\textbf{Methods note} (review \#19): the McNemar $p$-values are exact tests over the fixed 80-pair (concept $\times$ stub) probe grid; stub readback is a deterministic forward pass, so $p$ speaks to the exchangeability of the two instruments' outcomes across probes, not to run-to-run sampling variation (there is none). Lift-recovery (67\%) and write-sparsity (29\%) are ratios of per-cell means; the $2.32\times$ figure is the mean of per-cell lift-per-write ratios (\texttt{compose\_L22.py}, \texttt{efficiency()}).
